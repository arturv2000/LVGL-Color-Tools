...
exports.isPaletteExpression = isPaletteExpression;
...
const LV_COLOR_MAKE_FUNCTION_REGEX = new RegExp(`\\blv_color_make\\b\\s*\\(\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*\\)`, 'g');
const LV_COLOR_MAKE_MACRO_REGEX = new RegExp(`\\bLV_COLOR_MAKE\\b\\s*\\(\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*\\)`, 'g');
const LV_COLOR_MAKE_SINGLE_REGEX = new RegExp(`\\b(lv_color_make|LV_COLOR_MAKE)\\b\\s*\\(\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*\\)`);
const LV_PALETTE_MAIN_REGEX = new RegExp(`\\blv_palette_main\\b\\s*\\(\\s*(${PALETTE_ENUM_PATTERN})\\s*\\)`, 'g');
const LV_PALETTE_LIGHTEN_REGEX = new RegExp(`\\blv_palette_lighten\\b\\s*\\(\\s*(${PALETTE_ENUM_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*\\)`, 'g');
const LV_PALETTE_DARKEN_REGEX = new RegExp(`\\blv_palette_darken\\b\\s*\\(\\s*(${PALETTE_ENUM_PATTERN})\\s*,\\s*(${BYTE_ARGUMENT_PATTERN})\\s*\\)`, 'g');
...
function findLvglColorsInText(text, options) {
...
function isPaletteExpression(token) {
...
function createMakeOffsetMatch(match, format) {
...
function createPaletteMainOffsetMatch(match) {
...
function createPaletteVariantOffsetMatch(match, variant, maxLevel) {
...
function getLvglPatterns(options) {
...
function resolvePaletteColor(paletteName, variant, level) {
...
function toLvColorMake(color, useHexByteArguments, useMacroName) {
